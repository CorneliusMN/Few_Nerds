...
def train(args, train_dataset, model, tokenizer, labels, pad_token_label_id):
    ...
    for epoch in train_iterator:
        with open("output_supervised.txt", "a", encoding="utf-8") as writer:
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    ...
    for true, pred in zip(flat_labels, flat_preds):
    ...
    # Log and write the confusion counts per label
    logger.info("***** Per-label Confusion Matrix *****")
    with open(args.output_file_name, "a", encoding="utf-8") as writer:
        writer.write("***** Per-label Confusion Matrix *****\n")
        for label in sorted(label_metrics.keys()):
            counts = label_metrics[label]
            logger.info("Label: %s | TP: %d | FP: %d | TN: %d | FN: %d",
