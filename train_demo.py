...
# #     # ADDED
# #     if not opt.use_sampled_data:
# #         opt.train = './train_art_2_1.jsonl'
# #         opt.test = './test_art_2_1.jsonl'
# #         opt.dev = None  # Skip the dev (validation) file if you don't have one
# #     else:
# #         opt.train = './train_art_2_1.jsonl'
# #         opt.test = './test_art_2_1.jsonl'
# #         opt.dev = None  # Skip the dev (validation) file
...
#     # Load train data
#     train_data_loader = get_loader(opt.train, tokenizer, N=trainN, K=K, Q=Q, batch_size=batch_size, max_length=max_length, ignore_index=opt.ignore_index, use_sampled_data=opt.use_sampled_data)

#     # Only load validation data if opt.dev is not None
#     if opt.dev:
#         print('loading validation data...')
#         val_data_loader = get_loader(opt.dev, tokenizer, N=N, K=K, Q=Q, batch_size=batch_size, max_length=max_length, ignore_index=opt.ignore_index, use_sampled_data=opt.use_sampled_data)
#     else:
#         print('Skipping validation data (dev file is None).')
#         val_data_loader = None

#     # Load test data
#     test_data_loader = get_loader(opt.test, tokenizer, N=N, K=K, Q=Q, batch_size=batch_size, max_length=max_length, ignore_index=opt.ignore_index, use_sampled_data=opt.use_sampled_data)
...
#         framework.train(model, prefix, load_ckpt=opt.load_ckpt, save_ckpt=ckpt,
#                         val_step=opt.val_step, fp16=opt.fp16,
#                         train_iter=opt.train_iter, warmup_step=int(opt.train_iter * 0.1), val_iter=opt.val_iter, learning_rate=opt.lr, use_sgd_for_bert=opt.use_sgd_for_bert)
#     else:
#         ckpt = opt.load_ckpt
...
# python3 train_demo.py --mode inter --use_sampled_data --train_file "./train_art_2_1.jsonl" --test_file "./test_art_2_1.jsonl" --N 2 --K 1 --trainN 2 --batch_size 4 --lr 1e-4 --max_length 100
...
# other useful arguments: --val_iter (default 100, how many to evaluate on) --val_step (default 20, after how many training iterations)
